...
class PositionalTextEmbeddings:
    ...
    def __init__(
        ...
        self.update_prompt(
            base_prompt, top_prompt, side_prompt, back_prompt, front_prompt
        )
    ...
    def get_text_embedding(self, vertical_angle, horizontal_angle):
        ...
        # horizontal_angle = torch.where(
        #     horizontal_angle < 0, horizontal_angle + 360, horizontal_angle
        # )
        ...
        if self.positional_prompting == "discrete":
            ...
            if 0 < horizontal_angle <= 90:
                text_embedding = (horiz) * self.side_embed + (
                    90 - horiz
                ) * self.front_embed
            elif 90 < horizontal_angle <= 180:
                text_embedding = (horiz - 90) * self.back_embed + (
                    180 - horiz
                ) * self.side_embed
            elif 180 < horizontal_angle <= 270:
                text_embedding = (horiz - 180) * self.side_embed + (
                    270 - horiz
                ) * self.back_embed
            else:  # 270 < horizontal_angle <= 360:
                text_embedding = (horiz - 270) * self.front_embed + (
                    360 - horiz
                ) * self.side_embed

            text_embedding = text_embedding / 90.0
            text_embedding = (
                vert * text_embedding + (90 - vert) * self.top_embed
            ) / 90.0
        else:
