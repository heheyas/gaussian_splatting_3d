"""Stable Diffusion helpers, adapted from nerfstudio https://github.com/nerfstudio-project/nerfstudio/blob/HEAD/nerfstudio/models/generfacto.py"""
...
class StableDiffusion(nn.Module):
    ...
    def sds_loss(
        ...
        Returns:
            The loss
        """
        img = F.interpolate(image, (IMG_DIM, IMG_DIM), mode="bilinear").to(
            torch.float16
        )
        t = torch.randint(
            self.min_step, self.max_step + 1, [1], dtype=torch.long, device=self.device
        )
        latents = self.imgs_to_latent(img)
    ...
    def forward(
        ...
        Returns:
            The generated image.
        """
        return self.prompt_to_img(
            prompts, negative_prompts, num_inference_steps, guidance_scale, latents
        )
...
def generate_image(
    ...
    seed: int = 0,
    steps: int = 50,
    save_path: Path = Path("./tmp/generative/test_sd.png"),
):
